-- Dissector for PLATO Proprietary Protocol (PPP)
...
function plato_protocol.dissector(buffer, pinfo, tree)
  ...
  info_string = string.format("TargetAddr = 0x%02x     Length = %3d     Type = %s", buffer(0,1):int(), msg_length, packet_type_string) 
  ...
  if length > 12 then    
	...
	payloadSubtree:add(data_field, buffer(12, msg_length))
	payloadSubtree:add(data_crc, buffer(12 + msg_length, 1))
  end
...
function get_ccd_side(ccd_side_flag)
  local ccd_side = "Unknown"
      ...
      if ccd_side_flag == 0 then ccd_side = "Left/E"
  elseif ccd_side_flag == 1 then ccd_side = "Right/F" end
...
function get_packet_type_name(packet_type)
  local packet_type_name = "Unknown"
      ...
      if packet_type == 0 then packet_type_name = "Data packet"
  elseif packet_type == 1 then packet_type_name = "Overscan data packet"
  elseif packet_type == 2 then packet_type_name = "DEB houskeeping packet"
  elseif packet_type == 3 then packet_type_name = "AEB houskeeping packet" end
